...
- **MVP / product-architecture SPEC** - DIRECTED Pete 18:55 (coordinated gated plan, NOT rush-to-code). ✅ SPEC DONE (lane ac512471, `docs/working/JUN30_stocktrack_MVP_architecture_SPEC.md` 13baeba, pushed to Stock-Track): first slice = Inventory+Dashboard+Scan; entities products/categories/locations/installers/installations/customers/recalls/users; Flutter+Riverpod+Firebase(Brandon's OWN)+phone-cam scan; 9 owner-calls C1-C9 w/ recs; gated build seq. ✅ SPEC APPROVED by Pete 19:40 — all 9 calls locked (Firebase=Brandon's-own, phone-cam scan, slice1=Inventory+Dashboard+Scan, single-company, simple-roles-reserved, recalls=flag->affected-list, truck-implicit, customer-free-text-first, greenfield). NOW: Flutter SHELL PLAN executing (lane ad99d561, plan-only, no code) -> `docs/working/JUN30_stocktrack_FLUTTER_SHELL_PLAN.md`. ⛔ HARD GATE before any build/APK: Brandon creates his OWN Firebase project (config from HIS project only, never BP). Orch offered Pete the Firebase-setup walkthrough for Brandon. Sequence: shell-plan -> Brandon Firebase -> gated build (Inv+Dash+Scan) -> APK.
