...
- **MVP / product-architecture SPEC** - DIRECTED Pete 18:55 (coordinated gated plan, NOT rush-to-code). ✅ SPEC DONE (lane ac512471, `docs/working/JUN30_stocktrack_MVP_architecture_SPEC.md` 13baeba, pushed to Stock-Track): first slice = Inventory+Dashboard+Scan; entities products/categories/locations/installers/installations/customers/recalls/users; Flutter+Riverpod+Firebase(Brandon's OWN)+phone-cam scan; 9 owner-calls C1-C9 w/ recs; gated build seq. Delivered to Pete + doc. AWAITING Pete's SPEC-APPROVAL + calls on C1-C9. THEN Flutter shell (gated also on Brandon's own Firebase). Risks flagged: offline last-write-wins; scan-direction UI must be explicit.
